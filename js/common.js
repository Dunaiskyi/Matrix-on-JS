...
var config = {
  amount: 0,  // of columns going down (it will top at this number)
  speed: 80,   // time between updates in ms (lower = faster)
  size: 15,    // in px
  minLength: 40,
  maxLength: 50,
  firstColor: '#fff',
  color: 'rgba(0, 255, 32, 1)',
  density: 5
}
...
Data.prototype = {
  update() {
    ...
    this.history.unshift(String.fromCharCode(32 + Math.floor(Math.random() * 90)))
  ...
  draw() {
    ...
    for (var i = 1, char; char = this.history[i]; i++) {
      ctx.fillStyle = 'rgba(0, 255, 32,' + (op-= 1/config.maxLength) + ')'
...
setInterval(function() {
  ...
  for (var a = 0; a < config.density; a++) {
    if (config.amount == 0 || datarray.length < config.amount) putData()
  }
